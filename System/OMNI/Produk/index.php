          ...
          <div class="col-xs-12">
            <div class="card">
              ...
              <div class="card-body">
                ...
                require_once '../RequestAPI/shopee-get-product-info.php';
                require_once '../Process/tokopedia-addproduct.php';
                require_once '../Process/shopee-addproduct.php';
                ...
                $shopeeProducts = getShopeeProducts();

                $addNewProduct = new ProdukShopee();
                $result = $addNewProduct->processShopee($shopeeProducts);
                if ($result['status'] === 'success') {
                  echo "Data berhasil disimpan!";
                } else {
                  echo "Terjadi kesalahan saat menyimpan data.";
                }
                $query = "SELECT * FROM productomni WHERE Status != 2";
                  ...
                  <?php while ($product = mysqli_fetch_assoc($result)): ?>
                    <div class="row body-table">
                      <div class="col-lg-1 col-xs-1">
                        ...
                        ?>
                        <img class="img-fluid" src="<?php echo htmlspecialchars($imageUrl); ?>" alt="product_image"
                          width="75px">
                      </div>
                          ...
                          <div style="width:80%;">
                              <div class="input-group">
                                  ...
                                  <input class="form-control price-input" 
                                        ...
                                        data-available-in="<?php echo htmlspecialchars($product['AvailableIn']); ?>" />
                              </div>
                          </div>
                      </div>
                    ...
                    <div class="col-lg-2 col-xs-2"> <input class="form-control digits" type="text"
                    ...
                    <button type="button" class="border-grey border b-r-50" ><i class="icon-trash"></i></button>
                      
                      <!-- <select class="form-select" id="validationDefault04" onchange="handleAction(this, <?php echo htmlspecialchars($product['ProductID']); ?>)">
                    ...
                    function handleAction(select, productId) {
                      ...
                      } else if (select.value === "nonaktif") {
                        alert("Produk akan dinonaktifkan");
                      }
                    ...
                    <?php if (!empty($product['ChildID'])): ?>
                      <div class="row" style="margin-top: 10px; margin-left: 0px;">
                        ...
                        <div class="collapse mt-2" id="<?php echo $product['ProductID']; ?>"> <ul class="list-group"> <?php
                           $imgFolder = '../Produk/ProductImg/';
                           ...
                           while ($variant = mysqli_fetch_assoc($variantResult)):
                            ...
                            <li class="list-group-item">
                              ...
                              <div class="col-lg-1 col-xs-1">
                                <div class="flex-grow-1 icon-state">
                                  <label class="switch"> <input type="checkbox" class="status" name="status" 
              ...
              <script>
                $(document).ready(function () {
                  $('.price-input').on('blur', function () {
                    const newPrice = $(this).val(); // Ambil nilai harga baru
                    const productID = $(this).data('id'); // Ambil ProductID
                    const availableIn = $(this).data('available-in'); // Ambil platform (tokopedia/shopee)

                    console.log('New Price:', newPrice);
                    console.log('Product ID:', productID);
                    console.log('Available In:', availableIn);

                    // Validasi input
                    if (!isNaN(newPrice) && newPrice.trim() !== "" && productID !== 'ID tidak tersedia') {
                      // Panggil fungsi universal untuk update harga
                      updateProductPrice(productID, newPrice, availableIn);
                    } else {
                ...
                function updateProductPrice(productID, newPrice, availableIn) {
                  let url = '';
                  let data = {};

                  if (availableIn === 'tokopedia') {
                    url = '../RequestAPI/tokopedia-update-price.php';
                    data = {
                      product_id: productID,
                      new_price: newPrice
                    };
                  } else if (availableIn === 'shopee') {
                    url = '../RequestAPI/shopee-update-price.php';
                    data = {
                      item_id: productID,
                      new_price: newPrice
                    };
                ...
                $(document).ready(function () {
                  $('.stock-input').on('blur', function () {
                    ...
                    if (!isNaN(newStock) && newStock.trim() !== "" && productID !== 'ID tidak tersedia') {
                      // Panggil fungsi universal untuk update harga
                      updateProductStockPHP(productID, newStock, availableIn);
                    } else {
                      alert("Harap masukkan harga yang valid.");
                    }
                  });
                ...
                function updateProductStockPHP(productID, newStock, availableIn) {
                  let url = '';
                  let data = {};
                  ...
                  if (availableIn === 'tokopedia') {
                    ...
                    url = '../RequestAPI/shopee-update-stock.php';
                    data = {
                      item_id: productID,
                      new_stock: newStock
                    };
                ...
                document.querySelectorAll('.status').forEach(function (checkbox) {
                    checkbox.addEventListener('change', function () {
                        ...
                        var availableIn = this.getAttribute('data-available-in');

                        var url;
                        if (availableIn === 'shopee') {
                            url = isChecked ? '../RequestAPI/shopee-update-status-active.php' : '../RequestAPI/shopee-update-status-inactive.php';
                        } else if (availableIn === 'tokopedia') {
                            url = isChecked ? '../RequestAPI/tokopedia-set-active.php' : '../RequestAPI/tokopedia-set-inactive.php';
                        } else {
                            console.error('Unknown platform: ' + availableIn);
                            return;
                        }
