...
class ProdukTokopedia
{
    ...
    public function simpanProduk($product)
    {
        ...
        if ($hasilCek->num_rows > 0) {
            $queryUpdate = "UPDATE productomni SET ProductName = ?,SKU = ?, Img = ?, Description = ?, ModalPrice = ?, SellingPrice = ?, isParent = ?, ChildID = ?, StockValue = ?, MainStock = ?, CreatedOn = ?, LastUpdated= ?, AvailableIn = ?, Status = ? WHERE ProductID = ?";
            $stmt = $this->conn->prepare($queryUpdate);
            $stmt->bind_param("ssssdsssiisssss", $productName, $sku, $img, $Description, $modalPrice, $sellingPrice, $isParent, $childrenIDs, $stockvalue, $mainstock, $CreatedOn, $LastUpdated, $availablein, $status, $productId);
        } else {
            $queryInsert = "INSERT INTO productomni (ProductID, Img, ProductName, SKU, Description, ModalPrice, SellingPrice, isParent, ChildID, StockValue, MainStock, CreatedOn, LastUpdated, AvailableIn,Status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $this->conn->prepare($queryInsert);
            $stmt->bind_param("ssssddssiisssss", $productId, $img, $productName, $sku, $Description, $modalPrice, $sellingPrice, $isParent, $childrenIDs, $stockvalue, $mainstock, $CreatedOn, $LastUpdated, $availablein, $status);
        }
        ...
        if (isset($variants['data']['children']) && !empty($variants['data']['children'])) {
            foreach ($variants['data']['children'] as $index => $variantx) {
                ...
                $skuvariant = isset($variantx['sku']) ? $variantx['sku'] : 'Unknown';
                ...
                if (!empty($img)) {
                    ...
                    if ($hasilVariantCek->num_rows > 0) {
                        $queryUpdateVariant = "UPDATE productvariantomni SET ProductName = ?,SKU = ?, Img = ?, ModalPrice = ?, SellingPrice = ?, StockValue = ?, MainStock = ?, ReserveStock = ?, Status = ? WHERE ProductIDVariant = ?";
                        $stmtVariant = $this->conn->prepare($queryUpdateVariant);
                        $stmtVariant->bind_param("sssiiiissi", $prodname, $skuvariant, $img, $pricemodal, $pricevariant, $stockvaluevariant, $mainstockvariant, $mainstockvariant, $statusvariant, $productid);
                    } else {
                        $queryInsertVariant = "INSERT INTO productvariantomni (ProductIDVariant, ProductName, SKU, Img, ModalPrice, SellingPrice, StockValue, MainStock, ReserveStock, Status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        $stmtVariant = $this->conn->prepare($queryInsertVariant);
                        $stmtVariant->bind_param("isssiiiiss", $productid, $prodname, $skuvariant, $img, $pricemodal, $pricevariant, $stockvaluevariant, $mainstockvariant, $mainstockvariant, $statusvariant);
                    }
