                      ...
                      <div class="card-body">
                        ...
                        <div class="tab-content" id="myTabContent">
                          <div class="tab-pane fade show active" id="unpaid" role="tabpanel">
                            <h3>Invoicing Pembelian</h3><small>Status : Unpaid</small>
                            <br><br>
                            <div class="table-responsive custom-scrollbar user-datatable">
                              <table class="display" id="basic-12">
                                ...
                                <tbody>
                                  ...
                                  $queryR = "SELECT ri.RCV_InvoiceID, ri.CreatedOn, r.PurchaseOrderID, ri.DPP, ri.PPN, ri.TotalAmount, ri.TaxInvoiceNumber
                                      FROM receptioninvoiceheader ri, receptionheader r
                                      WHERE ri.Status=0
                                            AND ri.ReceptionID = r.ReceptionID
                                            ORDER BY ri.RCV_InvoiceID DESC";
                                  $resultR = mysqli_query($conn, $queryR);
                                  while ($rowR = mysqli_fetch_array($resultR)) {
                                    echo '<tr>
                                          <td>' . $rowR["RCV_InvoiceID"] . '</td>
                                          <td>' . $rowR["CreatedOn"] . '</td>
                                          <td>' . $rowR["TaxInvoiceNumber"] . '</td>
                                          <td>' . number_format($rowR["TotalAmount"], 0, '.', ',') . '</td>
                                          <td> 
                                            <ul> ';
                                    if ($can_update) {
                                      echo '<button onclick="viewInvoicing(this)" value="' . $rowR["RCV_InvoiceID"] . '" type="button" class="light-card border-primary border b-r-10"><i class="fa fa-eye txt-primary"></i></button>';
                                    }
                                    echo ' 
                                            </ul>
                                          </td>
                          ...
                          <div class="tab-pane fade" id="paid" role="tabpanel">
                            <h3>Invoicing Pembelian</h3><small>Status : Paid</small>
                            <br><br>
                            <div class="table-responsive custom-scrollbar user-datatable">
                              <table class="display" id="basic-1">
                                ...
                                <tbody>
                                  <?php
                                  $queryR = "SELECT ri.RCV_InvoiceID, ri.CreatedOn, r.PurchaseOrderID, ri.DPP, ri.PPN, ri.TotalAmount
                                      FROM receptioninvoiceheader ri, receptionheader r
                                  ...
                                  while ($rowR = mysqli_fetch_array($resultR)) {
                                    echo '<tr>
                                        <td>' . $rowR["RCV_InvoiceID"] . '</td>
                                        <td>' . $rowR["CreatedOn"] . '</td>
                                        <td><a href="viewPurchaseOrder.php?id=' . $rowR["PurchaseOrderID"] . '">' . $rowR["PurchaseOrderID"] . '</a></td>
                                        <td>' . number_format($rowR["DPP"], 0, '.', ',') . '</td>
                                        <td>' . number_format($rowR["PPN"], 0, '.', ',') . '</td>
                                        <td>' . number_format($rowR["TotalAmount"], 0, '.', ',') . '</td>
                                        <td> 
                                          <ul> 
                                            <button onclick="viewInvoicing(this)" value="' . $rowR["RCV_InvoiceID"] . '" type="button" class="light-card border-primary border b-r-10"><i class="fa fa-eye txt-primary"></i></button>
                                          </ul>
                                        </td>
                                      </tr>';
                                  }
