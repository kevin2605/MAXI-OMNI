...
<head>
    ...
    $query = "SELECT tPOBahan FROM useraccesslevel WHERE UserID = '$userID'";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);

    // Cek akses CRUD dan tentukan apakah akses diizinkan
    $hasCRUDAccess = strpos($row['tPOBahan'], 'C') !== false || // Create
        strpos($row['tPOBahan'], 'R') !== false || // Read
        strpos($row['tPOBahan'], 'U') !== false || // Update
        strpos($row['tPOBahan'], 'D') !== false;  // Delete
    ...
    <script>
        ...
        function viewPO(str) {
            document.location = "viewPOMaterial.php?id=" + str.value;
        }
        function printPO(str) {
            var invoiceID = str.value;
            var url = "../Process/generate_POL_pdf.php?PurchaseOrderID=" + invoiceID;
                                    ...
                                    <div class="col-md-12">
                                        <div class="card">
                                            <div class="card-body">
                                                ...
                                                if (!empty($userID)) {
                                                    $query_access = "SELECT tPOBahan FROM useraccesslevel WHERE UserID = '$userID'";
                                                    $result_access = mysqli_query($conn, $query_access);

                                                    if ($result_access) {
                                                        $row_access = mysqli_fetch_assoc($result_access);
                                                        $access_level = $row_access['tPOBahan'];
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body dark-modal">
                                                                <div class="card-body custom-input">
                                                                    <form class="row g-3"
                                                                        action="../Process/createPOMaterial.php"
                                                                        method="POST">
                                                                        <div class="col-3">
                                                        ...
                                                        <div class="table-responsive custom-scrollbar user-datatable">
                                                            <table class="display" id="basic-12">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    $query_access = "SELECT tPOBahan FROM useraccesslevel WHERE UserID = '$creator'";
                                                                    $result_access = mysqli_query($conn, $query_access);
                                                                    $can_update = false;
                                                                    $can_print = false;
                                                                    if ($result_access) {
                                                                        ...
                                                                        if (strpos($access_level, 'U') !== false || strpos($creator, 'ADM') !== false) {
                                                                            $can_print = true;
                                                                        }
                                                                    } else {
                                                                        die("Error: Gagal mengambil data akses pengguna.");
                                                                    ...
                                                                    while ($rowPO = mysqli_fetch_array($resultSO)) {
                                                                        ...
                                                                        <ul>';
                                                                        if ($can_update) {
                                                                            echo '<button  onclick="viewPO(this)" type="button" style="margin-right: 5px;" class="light-card border-primary border b-r-10" value="' . $rowPO["PurchaseOrderID"] . '"><i class="fa fa-eye txt-primary"></i></button>';
                                                                        }
                                                                        echo '<button onclick="editPO(this)" type="button" style="margin-right: 5px;" class="light-card border-warning border b-r-10" value="' . $rowPO["PurchaseOrderID"] . '"><i class="fa fa-pencil-square-o txt-warning"></i></button>';
                                                                        if ($can_print && $rowPO["ApprovalBy"] != NULL) {
                                                                            echo '<button onclick="printPO(this)" type="button" style="margin-right: 5px;" class="light-card border-info border b-r-10" value="' . $rowPO["PurchaseOrderID"] . '"><i class="fa fa-print txt-info"></i></button>';
                                                                        } else if (!$can_print && $rowPO["ApprovalBy"] == NULL) {
                                                                            echo '<!-- Print button hidden due to conditions -->';
                                                                        }
                                                                        echo '</ul>
                                                                            </td>
                                                                        </tr>';
