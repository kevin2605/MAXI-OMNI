...
<head>
    ...
    $query = "SELECT SparePart FROM useraccesslevel WHERE UserID = '$userID'";
    $result = mysqli_query($conn, $query);
    $row = mysqli_fetch_assoc($result);

    $hasCRUDAccess = strpos($row['SparePart'], 'C') !== false || // Create
        strpos($row['SparePart'], 'R') !== false || // Read
        strpos($row['SparePart'], 'U') !== false || // Update
        strpos($row['SparePart'], 'D') !== false;  // Delete
    ...
    <script>
        ...
        function viewPart(str) {
            document.location = "viewSparepart.php?partcd=" + str.value;
        }

        function editPart(str) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.location = "editSparepart.php?partcd=" + str.value;
                }
            });
        }
    </script>
                        ...
                        <div class="page-title">
                            ...
                            if (isset($_GET["status"])) {
                                if ($_GET["status"] == "success") {
                                    ...
                                    echo '<div class="alert txt-success border-success outline-2x alert-dismissible fade show alert-icons" role="alert">
                                    <p><b> Selamat! </b>Spare Part berhasil disimpan ke database.</p>
                                    <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                                    </div>';
                                }
                            }
                                            ...
                                            <div class="card-body">
                                                <div class="row">
                                                    <div class="col-md-3">
                                                        <?php
                                                        $canUpdate = false;
                                                        $isDisabled = true; // Asumsi default tombol harus dinonaktifkan
                                                        
                                                        if (!empty($userID)) {
                                                            $query_access = "SELECT SparePart FROM useraccesslevel WHERE UserID = '$userID'";
                                                            $result_access = mysqli_query($conn, $query_access);

                                                            if ($result_access) {
                                                                $row_access = mysqli_fetch_assoc($result_access);
                                                                $access_level = $row_access['SparePart'];

                                                                if (strpos($access_level, 'C') !== false) {
                                                                    $canUpdate = true;
                                                                    $isDisabled = false; // Izinkan akses, tombol tidak dinonaktifkan
                                                                }
                                                            } else {
                                                        ...
                                                        <button class="btn btn-outline-primary" type="button" <?php echo $isDisabled ? 'disabled' : 'data-bs-toggle="modal" data-bs-target=".bd-example-modal-xl"'; ?>><i
                                                                ...
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body dark-modal">
                                                                        <div class="card-body custom-input">
                                                                            <form class="row g-3"
                                                                                action="../Process/createSparepart.php"
                                                                                method="POST">
                                                                                <div class="col-3">
                                                        ...
                                                        <div class="table-responsive custom-scrollbar user-datatable">
                                                            <table class="display" id="basic-12">
                                                                ...
                                                                <tbody id="material-table-body">
                                                                    ...
                                                                    $query_access = "SELECT SparePart FROM useraccesslevel WHERE UserID = '$creator'";
                                                                    $result_access = mysqli_query($conn, $query_access);
                                                                    $can_updatee = false;
                                                                    if ($result_access) {
                                                                        $row_access = mysqli_fetch_assoc($result_access);
                                                                    ...
                                                                    while ($row = mysqli_fetch_array($result)) {
                                                                        echo '
                                                                            <tr>
                                                                                <td>' . $row["Sequence"] . '</td>
                                                                                <td>' . $row["PartCD"] . '</td>
                                                                                <td><a href="sparepart-history.php?partname=' . urlencode($row["PartName"]) . '">' . $row["PartName"] . '</a></td>
                                                                                ...
                                                                                <td> 
                                                                                    <ul> 
                                                                                                    <button onclick="viewPart(this)" type="button" class="light-card border-primary border b-r-10" value="' . $row["PartCD"] . '"><i class="fa fa-eye txt-primary"></i></button>';
                                                                        if ($can_updatee) {
                                                                            echo '<button onclick="editPart(this)" type="button" class="light-card border-warning border b-r-10" value="' . $row["PartCD"] . '"><i class="icon-pencil-alt txt-warning"></i></button>';
                                                                        }
                                                                        echo '
                                                                                    </ul>
                                                                                </td>
