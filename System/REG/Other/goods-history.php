...
<head>
    ...
    <script>
        window.onload = function () {
            ...
            var prodcd = url.searchParams.get("prodcd"); // Ubah 'goods' menjadi 'prodcd'
            showSupportingGoods(prodcd);
        };

        function showSupportingGoods(x) {
            var detail = false;
            ...
            if (x != "") {
                ...
                xmlhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        document.getElementById("goodsDetail").innerHTML = this.responseText;
                    }
                };
                detail = true;
                xmlhttp.open("GET", "../Process/getSupportingGoodsDetail.php?prodcd=" + encodeURIComponent(x), true); // Ubah 'goods' menjadi 'prodcd'
                xmlhttp.send();

                if (detail == true) {
                    var xmlhttp2 = new XMLHttpRequest(); // Ubah nama variabel untuk menghindari konflik
                    xmlhttp2.onreadystatechange = function () {
                        if (this.readyState == 4 && this.status == 200) {
                            document.getElementById("flowHistory").innerHTML = this.responseText;
                        }
                    };
                    xmlhttp2.open("GET", "../Process/getSupportingGoodsFlowHist.php?prodcd=" + encodeURIComponent(x), true); // Ubah 'goods' menjadi 'prodcd'
                    xmlhttp2.send();
                }
            } else {
                alert("Produk kosong!");
                            ...
                            <div class="card">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        ?>
                                        <div class="row">
                                            <div class="col-md-4" id="goodsDetail">
                                                <!-- Detail barang penunjang akan muncul di sini -->
                                            </div>
                                            <div class="col-md-8" id="flowHistory">
                                                <!-- Histori barang penunjang akan muncul di sini -->
                                            </div>
                                        </div>
                                    </div>
                                </div>
