    ...
    <script>
        ...
        function approvePO(str) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.location = "../Process/eventApproveIPO.php?id=" + str.value + "&po=Material";
                }
            });
        }
        ...
        function rejectPO(str) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.location = "../Process/eventRejectIPO.php?id=" + str.value + "&po=Material";
                }
            });
        }
                                    ...
                                    <div class="col-xl-6 shipping-border">
                                        ...
                                        <div class="tab-content dark-field shipping-content"
                                            ...
                                            <div class="tab-pane fade show active" id="bill-wizard" role="tabpanel"
                                                ...
                                                <p class="f-light"></p>
                                                <?php
                                                $queryPO = "SELECT  po.PurchaseOrderID, po.CreatedOn, po.CreatedBy, po.DeliveryDate, po.ShippingAddress, po.Description, po.ApprovalStatus, 
                                                    po.ApprovalBy, po.ApprovalOn, s.SupplierNum, s.SupplierName, s.SupplierAdd, s.ContactName, s.ContactPhone, s.Telepon, po.Finish
                                                    FROM (importpurchaseorderheader po JOIN supplier s ON po.SupplierNum=s.SupplierNum) WHERE PurchaseOrderID='" . $_GET["id"] . "'";
                                    ...
                                    <div class="col-xl-6">
                                        ...
                                        <div class="overflow-auto">
                                            <table class="table table-striped">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $queryd = "SELECT pod.ItemCD, m.MaterialName, pod.Price, pod.Quantity, pod.QuantityReceived 
                                                    FROM importpurchaseorderdetail pod 
                                                    JOIN material m ON pod.ItemCD = m.MaterialCD 
                                                    WHERE pod.PurchaseOrderID = '" . $row["PurchaseOrderID"] . "'";
                                                    ...
                                                    while ($rowd = mysqli_fetch_array($resultd)) {
                                                        echo '<tr>
                                                            ...
                                                            <td>' . number_format($rowd["Quantity"], 0, '.', ',') . ' KG</td>
                                                            <td>' . number_format($rowd["QuantityReceived"], 0, '.', ',') . ' KG</td>
                                                            <td>' . number_format($rowd["Quantity"] - $rowd["QuantityReceived"], 0, '.', ',') . ' KG</td>        
                                                          </tr>';
                                                        echo '<tr>
                                                            <td colspan=7 class="collapse" id="' . $rowd["ItemCD"] . '">
                                                                <div class="card">
                                                                    <table class="table table-striped">
                                                                        ...
                                                                        <tbody>';
                                                        $queryp = "SELECT * FROM materialflowhistory WHERE ReferenceKey IN 
                                                        (SELECT ReceptionID FROM importreceptionheader WHERE PurchaseOrderID='" . $row["PurchaseOrderID"] . "' 
                                                        AND MaterialCD='" . $rowd["ItemCD"] . "')";
                                                        ...
                                                        while ($rowp = mysqli_fetch_array($resultp)) {
                                                            echo
                                                                '<tr>
                                                                    ...
                                                                    <td>' . $rowp["Date"] . '</td>
                                                                    <td>' . number_format($rowp["FlowIn"], 0, '.', ',') . '</td>
                                                                </tr>';
                                                        }
                                                        echo '              
                                                                        </tbody>
                                        ...
                                        <br>
                                        <div>
                                            <a class="btn btn-warning" href="purchasing-material.php">Back</a>
                                            <?php
